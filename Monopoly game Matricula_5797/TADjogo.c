...
void comprarPropriedade(Jogador* jogador, Localidade* localidade) {
    if (jogador->saldo >= localidade->custoCompra) {
        ...
        localidade->proprietario = jogador->id; // Atribuição correta

        // Adiciona a localidade ao vetor de propriedades do jogador
        for (int i = 0; i < MAX_PROPRIEDADES; i++) {
...
void MovimentoJogadores(Jogo* jogo, int a) {
    ...
    printf("%s se moveu de %d para %d - %s\n", jogador->nome, prePosicao, jogador->posicao, novaLoc->localidade->endereco);

    // Verificar se a localidade tem um proprietário
    int idProprietario = novaLoc->localidade->proprietario;
    if (idProprietario == -1) {
        char escolha;
        printf("Você está na propriedade %s. Deseja comprar esta propriedade? (s/n): ", novaLoc->localidade->endereco);
        scanf(" %c", &escolha);
        ...
        char escolha;
        printf("Você já possui a propriedade %s. Deseja construir algo? (s/n): ", novaLoc->localidade->endereco);
        scanf(" %c", &escolha);
        ...
        // Propriedade já tem um proprietário, então o jogador precisa pagar aluguel
        Jogador *proprietario = &jogo->jogadores[idProprietario];
        int aluguel = novaLoc->localidade->valorAluguel; // Supondo que o valor do aluguel é armazenado na `localidade`
    ...
    if (jogador->saldo >= localidade->valorAluguel) {
        ...
        for (int i = 0; i < numero_jogadores; i++) {
            if (jogadores[i].id == localidade->proprietario) {
                ...
                printf("%s pagou aluguel de %d para %s\n", jogador->nome, localidade->valorAluguel, jogadores[i].nome);
                return;
            }
        }
